...
export default function PoliciesPage() {
  ...
  function updateOne(updated: Policy) {
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <div className="space-y-4">
        {policies === null ? (
          ...
          policies.map((p, i) => (
            <PolicyCard key={p.id} policy={p} index={i} isAdmin={isAdmin} onChanged={updateOne} />
          ))
        )}
      </div>

      <CreatePolicyModal open={modalOpen} onClose={() => setModalOpen(false)} onCreated={refetch} />
    </div>
  );
}
