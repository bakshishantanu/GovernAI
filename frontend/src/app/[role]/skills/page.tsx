...
//: Past this many skills the fanned deck stops being charming and starts
//: being unreadable (overlapping tilted cards, a long horizontal scroll) —
//: the open question the design canvas flagged. Falls back to a plain grid.
const DECK_LIMIT = 8;
 ...
 * skills only (currently three, all VERIFIED); the fan holds up to
 * `DECK_LIMIT` skills before handing off to a plain responsive grid, so the
 * page keeps working as the registry grows rather than degrading silently.
 */
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        {asDeck && !loading && skills.length > 0 && (
      ...
      {loading ? (
        ...
        <div className="mt-10 grid grid-cols-2 place-items-center gap-6 py-6 sm:grid-cols-3 lg:grid-cols-4">
          {skills.map((skill, i) => (
            <SkillCard key={skill.id} skill={skill} index={i} onReadMore={() => setSelected(skill)} flat />
          ))}
        </div>
      )}
