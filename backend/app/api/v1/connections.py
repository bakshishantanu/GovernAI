...
async def save_connection(
    ...
    A connection is shared, org-wide state -- one person's save affects
    every agent in the org that needs it, not just the one they were
    looking at. Audited the same way every other governance-relevant
    action is, so "who changed our Jira integration, and when" is always
    ...
    await audit_service.log_connection_saved(
        user.org_id, user.id, requirement_key=requirement_key, label=payload.label
    )
    await db.commit()
...
async def delete_connection(
    ...
    await audit_service.log_connection_deleted(
        user.org_id, user.id, requirement_key=requirement_key
    )
    await db.commit()
