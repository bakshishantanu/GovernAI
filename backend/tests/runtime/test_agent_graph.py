...
async def test_agent_completes_without_any_tool_call():
    ...
    result = await run_agent(service, tools=[], goal="what is the answer?")
...
async def test_agent_calls_a_tool_then_produces_final_answer():
    ...
    result = await run_agent(service, tools=[_EchoTool()], goal="echo hi")
...
async def test_multiple_tool_calls_in_one_turn_all_execute():
    ...
    result = await run_agent(service, tools=[_EchoTool()], goal="echo a and b")
...
async def test_unknown_tool_call_does_not_crash_the_run():
    ...
    result = await run_agent(service, tools=[], goal="do something")
...
async def test_tool_exception_is_caught_and_reported_not_raised():
    ...
    result = await run_agent(service, tools=[_AlwaysFailTool()], goal="fail please")
...
async def test_max_steps_guard_stops_an_infinite_tool_calling_loop():
    ...
    result = await run_agent(service, tools=[_EchoTool()], goal="loop forever", max_steps=3)
