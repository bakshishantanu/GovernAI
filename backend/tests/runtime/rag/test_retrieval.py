...
async def test_search_never_surfaces_out_of_scope_content_even_when_highly_relevant():
...
async def test_search_with_no_permitted_scopes_returns_nothing():
    adapter = DocumentSearchAdapter()
    results = await adapter.search("policy engine", permitted_scopes=frozenset())
    assert results == []
...
async def test_get_document_returns_none_for_unknown_id():
    adapter = DocumentSearchAdapter()
    assert await adapter.get_document("DOC-999") is None


async def test_get_document_text_returns_full_original_text():
    adapter = DocumentSearchAdapter()
    text = await adapter.get_document_text("DOC-1")
