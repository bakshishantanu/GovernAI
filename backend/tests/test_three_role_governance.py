...
async def test_user_cannot_access_unassigned_agent(standard_user):
    ...
    with pytest.raises(HTTPException) as exc:
        await get_agent(agent_id=agent_id, service=agent_service, user=standard_user)
...
async def test_user_can_access_assigned_agent(standard_user):
    agent_id = uuid.uuid4()
    agent = make_agent(agent_id, standard_user.org_id, uuid.uuid4(), assigned_user_id=standard_user.id)

    agent_service = AsyncMock()
    agent_service.agent_repo.get_agent.return_value = agent

    res = await get_agent(agent_id=agent_id, service=agent_service, user=standard_user)
