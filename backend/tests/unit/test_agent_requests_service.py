...
from app.domain.agent_requests.service import AgentRequestService, RequestNotFoundError, RequestAlreadyClaimedError, InvalidStateTransitionError
...
async def test_create_request(request_service):
    org_id = uuid4()
    user_id = uuid4()
    req = await request_service.create_request(org_id, user_id, "Test title", "Test description", ["skill_1"])
